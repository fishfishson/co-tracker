...
def main(args):
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
        ...
        for frame_idx, object_ids, masks in predictor.propagate_in_video(state):
            ...
            os.makedirs(os.path.join(args.video_path, "../seg"), exist_ok=True)
            os.makedirs(os.path.join(args.video_path, "../vis_seg"), exist_ok=True)

            img = loaded_frames[frame_idx]
            for obj_id, mask in mask_to_vis.items():
                mask_img = np.zeros((height, width), np.uint8)
                mask_img[mask] = 255
                cv2.imwrite(os.path.join(args.video_path, "../seg", f"{frame_idx:04d}.jpg"), mask_img)
                mask_img = np.zeros((height, width, 3), np.uint8)
                mask_img[mask] = color[(obj_id + 1) % len(color)]
                img = cv2.addWeighted(img, 1, mask_img, 0.2, 0)
                cv2.imwrite(os.path.join(args.video_path, "../vis_seg", f"{frame_idx:04d}.jpg"), img)
                break # only visualize the first object
